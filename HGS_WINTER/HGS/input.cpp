...
typedef struct
{
	...
	BYTE aKeyStateOld[NUM_KEY_MAX]; //前回入力情報
}KeyBoard;
...
typedef struct
{
	...
	DIJOYSTATE aOldState; //前回の入力情報を保存
}JoyPad;
...
int g_LastDevice; //最後の入力があったデバイス
...
void UpdateKeyboard(void)
{
	...
	if (SUCCEEDED(g_KeyBoard.pDevKeyboard->GetDeviceState(sizeof(aKeyState), &aKeyState[0])))
	{
		...
		{
			g_KeyBoard.aKeyStateOld[nCntKey] = g_KeyBoard.aKeyState[nCntKey]; //前回情報の保存;
			...
			//最終入力チェック
			if (g_KeyBoard.aKeyStateOld[nCntKey] != g_KeyBoard.aKeyState[nCntKey])
			{
				g_LastDevice = LAST_KEYBOARD;
			}
		}
	}
	else
...
void UpdateJoyPad(void)
{
	for (int nCntPlayer = 0; nCntPlayer < MAX_PLAYER; nCntPlayer++)
	{
		...
		if (SUCCEEDED(g_aJoyPad[nCntPlayer].pDevJoypad->GetDeviceState(sizeof(JoyKey), &JoyKey)))
		{
			...
			//前回の入力情報を保存する
			g_aJoyPad[nCntPlayer].aOldState = g_aJoyPad[nCntPlayer].aKeyState;
			...
			{
				if (g_aJoyPad[nCntPlayer].aOldState.rgbButtons[nCntButton] != g_aJoyPad[nCntPlayer].aKeyState.rgbButtons[nCntButton])
				{
					g_LastDevice = LAST_JOYPAD;
				}
			}
		}
	}
...
//  ボタン情報の取得
//==========================================
bool GetJoyPadButtonPress(int nKey, int nPlayer) //プレス
{
	return (g_aJoyPad[nPlayer].aKeyState.rgbButtons[nKey] & 0x80) ? true : false;
}

bool GetJoyPadButtonTrigger(int nKey, int nPlayer) //トリガー
{
...
D3DXVECTOR3 GetStickR(int nPlayer) //右
{
	...
	stick.z = (float)g_aJoyPad[nPlayer].aKeyState.lRz / 1000.0f;
