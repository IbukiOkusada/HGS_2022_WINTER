...
void DrawMeshWall(void)
{
	...
	for (int nCntWall = 0; nCntWall < MAX_MESHWALL; nCntWall++)
	{
		...
		{//使用されているとき
			//ワールドマトリックスの初期化
			D3DXMatrixIdentity(&g_aMeshWall[nCntWall].Polygon.mtxWorld);

			//向きを反映
			D3DXMatrixRotationYawPitchRoll(&mtxRot, g_aMeshWall[nCntWall].Polygon.rot.y, g_aMeshWall[nCntWall].Polygon.rot.x, g_aMeshWall[nCntWall].Polygon.rot.z);
			D3DXMatrixMultiply(&g_aMeshWall[nCntWall].Polygon.mtxWorld, &g_aMeshWall[nCntWall].Polygon.mtxWorld, &mtxRot);

			//位置を反映
			D3DXMatrixTranslation(&mtxTrans, g_aMeshWall[nCntWall].Polygon.pos.x, g_aMeshWall[nCntWall].Polygon.pos.y, g_aMeshWall[nCntWall].Polygon.pos.z);
			D3DXMatrixMultiply(&g_aMeshWall[nCntWall].Polygon.mtxWorld, &g_aMeshWall[nCntWall].Polygon.mtxWorld, &mtxTrans);

			//ワールドマトリックスの設定
			pDevice->SetTransform(D3DTS_WORLD, &g_aMeshWall[nCntWall].Polygon.mtxWorld);
			...
			//テクスチャの設定
			pDevice->SetTexture(0, *SetTexture(g_aMeshWall[nCntWall].nTextype));
...
bool CollisionMeshWall(D3DXVECTOR3 *pPos, D3DXVECTOR3 *pPosOld, D3DXVECTOR3 *pMove)
{
	...
	for (int nCntWall = 0; nCntWall < MAX_MESHWALL; nCntWall++, pMeshWall++)
	{
		...
		{
			if (pMeshWall->Polygon.rot.y == 0.0f * D3DX_PI)
			{//壁が正面を向いている場合(Z軸がマイナスの方を見ている)
				if (pPos->y >= pMeshWall->Polygon.pos.y &&
					pPos->y < pMeshWall->Polygon.pos.y + pMeshWall->Polygon.fHeight * 2 * pMeshWall->nHeightMesh&&
					pPos->x >= pMeshWall->Polygon.pos.x - pMeshWall->Polygon.fWidth * pMeshWall->nWidthMesh &&
					pPos->x <= pMeshWall->Polygon.pos.x + pMeshWall->Polygon.fWidth * pMeshWall->nWidthMesh &&
					pPosOld->z < pMeshWall->Polygon.pos.z &&
					pPos->z >= pMeshWall->Polygon.pos.z)
				{//当たっている場合
					bCollision = true;
				}
			}
			if (pMeshWall->Polygon.rot.y == 0.5f * D3DX_PI)
			{//壁が右を向いている場合(X軸がプラスの方を見ている)

				if (pPos->y >= pMeshWall->Polygon.pos.y &&
					pPos->y < pMeshWall->Polygon.pos.y + pMeshWall->Polygon.fHeight * 2 * pMeshWall->nHeightMesh&&
					pPos->z >= pMeshWall->Polygon.pos.z - pMeshWall->Polygon.fWidth * pMeshWall->nWidthMesh &&
					pPos->z <= pMeshWall->Polygon.pos.z + pMeshWall->Polygon.fWidth * pMeshWall->nWidthMesh &&
					pPosOld->x < pMeshWall->Polygon.pos.x &&
					pPos->x >= pMeshWall->Polygon.pos.x)
				{//当たっている場合
					bCollision = true;
				}
			}
			...
			if (pMeshWall->Polygon.rot.y == 1.0f * D3DX_PI)
